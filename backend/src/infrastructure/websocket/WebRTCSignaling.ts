...
export class WebRTCSignaling {
    ...
    socket.on(
      ...
      ({ roomId }: LeaveRoomPayload) => {
        try {
          ...
          this.logger.error(
            `Leave room failed: ${String(error)}`
          );
        }
      }
    );
  ...
  private registerIceCandidate(socket: Socket): void {
    socket.on(
      ...
      ({ to, candidate }: IceCandidatePayload) => {
        try {
          this.logger.debug(
            `ICE Candidate: ${socket.id} → ${to}`
          );
          ...
          this.logger.error(
            `ICE Candidate failed: ${String(error)}`
          );
        }
      }
    );
