...
import { GetConsultationByPipelineQueryDTO } from '../dtos/consultation-command.dto';
...
export class GetConsultationByPipelineUseCase {
  constructor(private readonly consultationRepo: IConsultationRepository) {}

  public async execute(
    dto: GetConsultationByPipelineQueryDTO,
  ): Promise<Result<ConsultationResponseDTO>> {
    try {
      const consultation = await this.consultationRepo.findByAcquisitionPipelineId(
        dto.acquisitionPipelineId,
      );
      if (!consultation) {
        throw new ConsultationNotFoundException(dto.acquisitionPipelineId);
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'An unexpected error occurred while fetching consultation by pipeline';
