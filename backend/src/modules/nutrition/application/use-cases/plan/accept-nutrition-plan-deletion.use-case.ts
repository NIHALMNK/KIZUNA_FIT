...
export class AcceptNutritionPlanDeletionUseCase {
  ...
  async execute(planId: string, requestingClientId: string): Promise<NutritionPlanResponseDto> {
    ...
    const acceptResult = plan.acceptDeletion();
    if (acceptResult.isFailure) {
      throw new ValidationError(acceptResult.error || 'Failed to accept plan retirement.');
    }
