...
export class ListSentOffersUseCase {
  ...
  public async execute(dto: GetOffersQueryDTO): Promise<Result<PaginatedOffersResponseDTO>> {
    try {
      ...
      const result = await this.offerRepo.findByTrainerId(dto.userId, {
        status: dto.status,
        sort: dto.sort,
        limit,
        offset,
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'An unexpected error occurred while retrieving sent offers';
