...
import { useUpdateExercise } from '../../application/mutations/useWorkoutMutations';
...
import { Dumbbell, Plus, Trash2, X, Save, Layers, Image as ImageIcon } from 'lucide-react';
...
export const EditExerciseModal: React.FC<EditExerciseModalProps> = ({
  ...
  useEffect(() => {
    if (exercise) {
      ...
      setPrimaryMuscleGroup(exercise.primaryMuscleGroup || PrimaryMuscleGroup.CHEST);
      setSecondaryMuscleGroups(exercise.secondaryMuscleGroups || []);
      setEquipment(exercise.equipment || EquipmentType.BARBELL);
      setDifficulty(exercise.difficulty || DifficultyLevel.INTERMEDIATE);
      setCaloriesPerMinute(exercise.caloriesPerMinute || 5);
      setInstructions(
        exercise.instructions && exercise.instructions.length > 0
          ? exercise.instructions.map((i) => i.instruction)
          : ['Assume the starting position with proper alignment.'],
      );

      const media = exercise.media || {};
      setThumbnailUrl(media.thumbnailUrl || null);
      setImageUrls(media.imageUrls || media.images || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateExerciseMutation.mutateAsync({
      ...
      setErrorMsg(err.message || 'Failed to update exercise.');
    }
  };
      ...
      <div className="relative w-full max-w-2xl bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-5 border-b border-[var(--color-border)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-[var(--color-heading)]">Edit Exercise</h2>
              <p className="text-xs text-[var(--color-text-secondary)]">
                Update details, execution steps, and media assets
              </p>
            </div>
          </div>
          <button
